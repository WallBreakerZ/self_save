...
<div><div>Related Topics</div><div><li>哈希表</li><li>链表</li><li>双指针</li></div></div><br><div><li>👍 1783</li><li>👎 0</li></div>
*/
  ...
  public class 相交链表{
      public static void main(String[] args) {
          ...
          ListNode listA = ListNode.buildListNode(2, 6, 4);
          ListNode listB = ListNode.buildListNode(1,5);
          solution.getIntersectionNode(listA,listB);
      }
      //leetcode submit region begin(Prohibit modification and deletion)
/**
 * Definition for singly-linked list.
 * public class com.zyh.leetcode.editor.cn.common.ListNode {
 *     int val;
 *     com.zyh.leetcode.editor.cn.common.ListNode next;
 *     com.zyh.leetcode.editor.cn.common.ListNode(int x) {
 *         val = x;
 *         next = null;
 *     }
 * }
...
public class Solution {
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        ...
        ListNode ap = headA;//先指向 a链表
        ListNode bp = headB;//先指向 b链表
        boolean firstA = true;//第一次指针指向另一个链表，防止死循环
        boolean firstB = true;//第一次指针指向另一个链表，防止死循环
        while (ap != null && bp != null) {
            ...
            bp = bp.next;
            if ( firstA && ap == null){
                //当ap率先走到尾节点时，将ap指向b链表
                ap = headB;
                firstA = false;
            }
            if (firstB && bp == null){
                //当bp率先走到尾节点时，将bp指向a链表
                bp = headA;
                firstB = false;
            }
            //解释：当双指针都将两个链表遍历了一遍之后，则ap走过了 m+n个节点，bp也走过了 m+n个节点，
            //相当于两个指针走过了同一条m+n长度的链表，如果期间有相相交的节点，则一定会同时遇到；
        }
        return null;
    }
}
